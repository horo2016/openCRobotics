...
int SonarModelArray(cv::Mat occmap,int Xr,int Yr,int Rangle,int SonarDist,int cellsize,float scale)
{
	...
	//printf("SonarDist:%d \n",SonarDist);
	//在新地图中使用画椭圆
	ellipse(mapgrid,Point(int(Xr),int(Yr)),Size(SonarDist, SonarDist), Rangle, -25, 25, Scalar(255), thicknesss);
	//Mat dst;
	//resize(src, dst, cv::Size(1000, 1000));
...
int LidarModelArray(cv::Mat occmap,int Xr,int Yr,int Rangle,int SonarDist,int cellsize,float scale)
{
	...
	//printf("SonarDist:%d \n",SonarDist);
	//在新地图中使用画椭圆
	ellipse(mapgrid,Point(int(Xr),int(Yr)),Size(SonarDist, SonarDist), Rangle, -0.6, 0.6, Scalar(255), thicknesss);
	//Mat dst;
	//resize(src, dst, cv::Size(1000, 1000));
	//创建一个mat用于存储新的坐标
	//std::vector<cv::Point2i> locations;   // output, locations of non-zero pixels 

...
move_cmd = [0xff, 0xff, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3A]

class ChassisException(Exception):
...
def _process_scan(raw):
    ...
    if new_scan == inversed_new_scan:
        raise ChassisException('New scan flags mismatch')
    check_bit = _b2i(raw[1]) & 0b1
    if check_bit != 1:
        raise ChassisException('Check bit not equal to 1')
...
class Chassis(object):
    ...
    def connect(self):
        ...
        try:
            ...
            raise ChassisException('Failed to connect to the sensor '
                                   'due to: %s' % err)
    ...
    def send_move(self,v,w):
        ...
        req =  x_speed+y_speed+ang_speed
        checksum = 0
        for v in struct.unpack('B'*len(req), req):
           checksum ^= v
        req += struct.pack('B', checksum)
        payload = HEADER_BYTE+HEADER_BYTE+Len+req
        self._serial_port.write(bytes(payload))
        #for i in payload:
            #print('%#x'%ord(i))
    ...
    def _read_descriptor(self):
        ...
        if len(descriptor) != DESCRIPTOR_LEN:
            raise ChassisException('Descriptor length mismatch')
        elif not descriptor.startswith(SYNC_BYTE + SYNC_BYTE2):
            raise ChassisException('Incorrect descriptor starting bytes')
    ...
    def _read_response(self, dsize):
        ...
        if len(data) != dsize:
            raise ChassisException('Wrong body size')
        return data
    ...
    def get_info(self):
        ...
        if dsize != INFO_LEN:
            raise ChassisException('Wrong get_info reply length')
        if not is_single:
            raise ChassisException('Not a single response mode')
        if dtype != INFO_TYPE:
            raise ChassisException('Wrong response data type')
    ...
    def iter_measurments(self, max_buf_meas=500):
        ...
        if status == _HEALTH_STATUSES[2]:
            ...
            status, error_code = self.get_health()
            if status == _HEALTH_STATUSES[2]:
                raise ChassisException('RPLidar hardware failure. '
                                       'Error code: %d' % error_code)
        ...
        if dsize != 5:
            raise ChassisException('Wrong get_info reply length')
        if is_single:
            raise ChassisException('Not a multiple response mode')
        if dtype != SCAN_TYPE:
            raise ChassisException('Wrong response data type')
        while True:
            raw = self._read_response(dsize)

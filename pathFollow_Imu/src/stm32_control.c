...
unsigned long tinestamp_dead =0;
/************************************
 * 串口数据发送格式共15字节
...
char data_check(unsigned char *buffer,unsigned short n_len)
{
	unsigned char csum;
	char ret= -1;
	int i;
	...
	if((buffer[0]==0xaa) && (buffer[1]==0xaa)){
		...
		for(i = 1;i < n_len;i ++){
			if(buffer[i]==0xaa && buffer[i+1]==0xaa && (i+10)>(n_len-1))
				DEBUG(LOG_ERR,"datas misiing lost %d \n",i);
			    ret =i;
...
int *stm_Loop()
{
	...
	{	
		...
		//	  printf("%x ",r_buffer[0]);
     		  char retrn = data_check(r_buffer,i_nread);
			  if(retrn == 0){
				int i;
				for(i=0;i<4;i++){
